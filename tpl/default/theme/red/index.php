...
if ($task_open) {
	...
	$arrRecommTaskLists = db_factory::query(sprintf("select task_id,task_title,task_cash,task_cash_coverage,username,work_num from %switkey_task where  task_status=2 order by is_top desc, start_time desc limit 21",TABLEPRE),1,$intIndexCacheTime);
}
/**
 * 商品列表
 */
if ($shop_open) {
	...
	//行业推荐任务
	$arrRecommGoodsLists = db_factory::query(sprintf("select service_id,title,price,unite_price,pic,username from %switkey_service where  service_status=2 order by is_top desc , on_time desc limit 8",TABLEPRE),1,$intIndexCacheTime);;
}
/**
 * 首页案例
 */
